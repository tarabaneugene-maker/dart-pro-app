...
- **h/t колонка**: жёлтые цифры хитов за текущий подход у активного игрока. У неактивного — серым цветом показываются хиты его предыдущего подхода (lastTurnHits).
- **Блокировка тапов**: если tentative hits (permanent + currentTurn) >= 3, сектор блокируется для ввода. В Classic — сразу, в American — только если соперник тоже закрыл (иначе можно набирать очки).
- **Live-предпросмотр очков**: totalPoints в табло игрока обновляется мгновенно при вводе/стирании хита, до нажатия OK, по той же логике начисления.
